...
import { getAllUsers } from '@/data/api/admin-api';
import { listAuthUsers } from '@/data/api/auth-api';
import {
    addGroupMembers,
    createGroupChat,
    deleteChatMessage,
...
import { ChatApiError } from '@/data/api/chat-http';
import { deleteNotificationByEventKey } from '@/data/api/notifications-api';
import { fetchChatParticipantSnapshots } from '@/data/api/profile-api';
import { getMyTeamRoster, getVisibleDirectory } from '@/data/api/teams-api';
...
export function useGdcChatInbox({ token, user }) {
  ...
  /** chatId → last list preview (persisted for cold start) */
  const previewCacheRef = useRef(/** @type {Record<string, Record<string, unknown>>} */ ({}));
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void loadPreviewCache(myId).then((cache) => {
      if (!cancelled) previewCacheRef.current = cache || {};
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    const next = { ...previewCacheRef.current };
    let changed = false;
    for (const t of threads) {
      if (!t?.id || !previewHasContent(t.threadPreview)) continue;
      const id = String(t.id);
      const slim = slimPreviewForCache(t.threadPreview);
      if (!slim) continue;
      next[id] = slim;
      changed = true;
    }
    ...
    const timer = setTimeout(() => {
      void savePreviewCache(myId, previewCacheRef.current);
    }, 450);
  ...
  const mapServerThreads = useCallback(
    (rawList, opts = {}) => {
      ...
      return rawList.filter((server) => !hiddenChatIds.has(String(server?.id ?? ''))).map((server) => {
        ...
        const cachedPreview = previewCacheRef.current[id] ?? null;
        const threadPreview = preserveOpenMessages
          ? old?.threadPreview ?? cachedPreview
          : old?.threadPreview ?? cachedPreview;
  ...
  const hydrateMissingThreadPreviews = useCallback(
    async (threadList) => {
      if (!token || !myId) return;
      const list = Array.isArray(threadList) ? threadList : [];
      const targets = list.filter((t) => {
        if (!t?.id) return false;
        if (previewHasContent(t.threadPreview)) return false;
      ...
      await Promise.all(
        targets.map(async (t) => {
          const chatId = String(t.id);
          try {
            const rows = await listChatMessages(token, chatId, { limit: 1 });
            if (!Array.isArray(rows) || !rows.length) return;
            const ui = mapApiMessageToUi(rows[0], myId);
            previewCacheRef.current[chatId] = slimPreviewForCache(ui) || ui;
            setThreads((prev) => {
              const next = prev.map((row) =>
                threadIdEquals(row.id, chatId) ? { ...row, threadPreview: ui } : row,
              );
              return sortThreadsByRecent(next);
            });
          } catch {
            /* ignore per-thread preview fetch */
          }
        }),
      );
    },
  ...
  const refreshThreads = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        void hydrateMissingThreadPreviews(mapped);
    ...
    [token, myId, mapServerThreads, hydrateChatParticipants, hydrateMissingThreadPreviews],
  );
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      let cancelled = false;
      (async () => {
        previewCacheRef.current = await loadPreviewCache(myId);
